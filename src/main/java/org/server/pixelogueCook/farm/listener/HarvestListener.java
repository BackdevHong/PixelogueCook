...
public class HarvestListener implements Listener {
    ...
    public void onBreak(BlockBreakEvent e) {
        ...
        if (data instanceof Ageable ag) {
            if (ag.getAge() >= ag.getMaximumAge()) {
                // 다 자란 일반 작물 → 랭크 드롭
                e.setDropItems(false);

                Grade grade = crops.drawGrade(FertilizerTier.NONE); // 비료 없음
                ItemStack gradeDrop = ItemUtil.makeGradeDrop(plugin, b.getType(), grade, FertilizerTier.NONE, e.getPlayer());
                ItemStack replantItem = new ItemStack(ct.replantItem, 1);
